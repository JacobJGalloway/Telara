...
- **Solution/build membership**: `Telara.slnx` holds every .NET project across the stack, frontend and backend alike — a new Island only joins it if it's itself a .NET project (e.g. another Blazor host). A non-.NET Island (React Native, etc.) does not go in `Telara.slnx`; instead it gets its own step in the build pipeline (e.g. an `npm` call) so it's still built/verified as part of the overall build.
