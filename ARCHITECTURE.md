...
- **2026-09-15:** `RegisterStation` now accepts `predecessorStationIds` (a list) and `isLoadingDock`, closing the gap `postman-station-line-seed.md` flagged (routing previously had no mutation and was stitched by hand via `003.SeedStationRouting-Demo.sql`). The list shape supports funneling multiple existing predecessors into one new successor today — each predecessor just gets its own `NextStationId` set to the new station, which the existing single-FK schema already allows — without a breaking API change once the registration form actually offers multi-predecessor selection. Fan-out (one station feeding more than one successor) is explicitly still unsupported: confirmed as a deliberate boundary, not a gap — `NextStationId` stays a single outgoing edge per station. If a real fan-out need shows up, it's modeled as a distinct Station variant/type that owns the one-to-many split as its own concern, not as a generalization of every station's routing.
- **2026-09-17:** `StationEquipment` stays one entity for both manual work areas (e.g. "Packing," "Labeling" — a human action, no telemetry) and real automated equipment, rather than splitting out a `WorkArea` concept — same "minimal and reversible" reasoning as `NextStationId` over a new `StationRoute` entity. `LastSensorReadingUtc` and the rest of `StationEquipment`'s columns are already nullable/optional, so a manual slot just never accrues sensor data through the same table, same `StationSequence`/lease/status machinery. This directly unblocks the Employee domain model (1.4/1.5) — `Employee`-to-slot assignment targets `StationEquipment` regardless of whether that slot is automated. **Planned natural iteration, not committed now:** once manual-vs-automated needs distinct properties (beyond a nullable field or two), split via the interface pattern already used for `Station`/`IStation` — `ManualStationEquipment`, `ControlledStationEquipment`, `AutomatedStationEquipment` all implementing `IStationEquipment` — rather than widening one class indefinitely. See `DECISIONS.md` for the forward-looking scope note.
- **2026-09-17:** A role's dashboard is not necessarily a distinct screen — it's whichever view is that role's default/most-used landing, and the left nav can point a role at a *shared* view owned by the role below it rather than a scaled-down copy. Concretely: Shift Manager's default landing is the line-level `/shift` view (stations as a single unit), but the left nav also exposes the same `/station/:stationId` view Station Supervisors land on by default, reused as-is for drill-down — not a separate Shift-Manager-flavored station screen. Reasoning: a Shift Manager coordinating a resolution with a Station Supervisor needs to see the *same* station-level picture the Supervisor is looking at (equipment status, active alerts, staffing), not a summary of it — the Manager's actual value-add at that point is resources the Supervisor can't reach (e.g. calling in outside staff on overtime), not a different data view. This same reuse-down-the-hierarchy pattern is expected to extend to Plant Director → Shift Manager drill-down in 1.4 or 1.5 (timing depends on availability) — a Director drilling into a line for the same reason a Manager drills into a station: coordinating with the role below on a resolution, not viewing separately-built Director-flavored line data. Which view is a role's *default* landing stays an open, revisitable choice per role (not architecturally fixed) — deliberately left open for post-launch UX feedback rather than decided now.
...
- **Cleanup needed: `Roles` seed data doesn't match `DESIGN.md`'s three roles.** `000.SeedRolesTable.sql` only seeds a single "Shift Supervisor" role, which isn't Station Supervisor, Shift Manager, or Plant Director — it predates the 2026-09-17 role-hierarchy decisions above. Left as-is for now (2026-09-17: user's own demo login stays "Shift Supervisor") pending a follow-up pass to reseed `Roles` with the actual three, and scale `001.SeedUsersTable-Demo.sql` from one demo user up to four (the existing user plus one new seeded user per remaining role) so each role can be logged into and checked independently.
- **Resolved 2026-09-15:** fan-out (one station splitting into more than one successor) was flagged below as a known limitation; it's now a confirmed non-goal rather than an open question — see "Decisions Made This Sprint." A station cross-feeding multiple downstream lines, if ever needed, gets a dedicated Station variant that owns that split, not a generalized `StationRoute` edge table.
